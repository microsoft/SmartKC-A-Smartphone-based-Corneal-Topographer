...
def get_color_palette(pal_name = 'palette_rain.png'):
    # getting color palette for each mire visualization
    pal = cv2.cvtColor(cv2.imread(pal_name), cv2.COLOR_BGR2RGB)
    temp = []
    for idx in range(0, pal.shape[1], 20):
        temp.append(list(pal[40,idx, :]))
    color_pal = []

    i ,j = 0, len(temp)-1
    while i < j:
        color_pal.append(temp[i])
        color_pal.append(temp[j])
        i += 1
        j -= 1
...
global color_pal
# getting color palette for each mire visualization
color_pal = get_color_palette(pal_name = 'palette_rain.png')

def plot_color(img, points, color_pal): 
    idx = 0
    for point in points:
        if len(points) >= len(color_pal):
            break
        img[int(point[1]), int(point[0]), :] = color_pal[idx]
        idx += 1    
    return img
...
def plot_color_rb(img, points):
    ...
    for idx, point in enumerate(points):
        if idx%2 == 0:
            img[int(point[1]), int(point[0]), :] = [0, 255, 255] # [255, 0, 0]
        else:
            img[int(point[1]), int(point[0]), :] = [0, 0, 255]
