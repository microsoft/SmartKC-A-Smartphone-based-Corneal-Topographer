...
public class ZoomageView extends AppCompatImageView implements OnScaleGestureListener {
    ...
    private void drawCross(Canvas canvas) {
        if (lastTouchX != -1 && lastTouchY != -1) {
            ...
            float[] mat = reset ? startValues : matrixValues;
//            Float x = lastTouchX*mat[Matrix.MSCALE_X] + mat[Matrix.MTRANS_X];
//            Float y = lastTouchY*mat[Matrix.MSCALE_Y] + mat[Matrix.MTRANS_Y];
            Float x= lastTouchX;
            Float y = lastTouchY;
            canvas.drawLine(x-20, y-20, x+20, y+20, myPaint);
            canvas.drawLine(x-20, y+20, x+20, y-20, myPaint);
            reset = false;
        }
    }

    public PointF getCrossCoors() {
        if (startValues == null) {
            // User hasn't interacted with image
            return new PointF(-1, -1);
        }

        Float x = lastTouchX;
        Float y = lastTouchY;
        return new PointF((x - bounds.left)/(bounds.right-bounds.left), (y - bounds.top)/(bounds.bottom-bounds.top));
    }
    ...
    public void setCross() {
        lastTouchX = getWidth()/2;
        lastTouchY = getHeight()/2;
        invalidate();
    }
