...
`python main.py --n_mires 22 --working_distance 75.0 --camera_params "4.8 6.4 4.755" --model_file ./data/ring_distribution.txt --base_dir data/sample_images/ --center_selection auto --mire_seg_method dl --mire_loc_method graph_cluster --verbose`

Please place your csv files containing meta data generated by the smart kc app in the `base_dir` if you want the pipeline to automatically pick up the center values as marked in the smartkc app

There are 2 arguments which control how the segmentation and mire localization is done. 

(1) `--mire_seg_method` determines how the mires are segmented. Currently, we support 2 methods for segmentation, `image_proc` which does segmentation via traditional image processing, and `dl` which does image segmentation using a U-Net based segmentor model.

(2) `--mire_loc_method` determines how mires are localized. Currently, we support 2 methods - `radial_scan` which determines mire locations via the radial-scanning algorithm mentioned in the paper, and `graph_cluster` which applies graph-based connected-component search to locate mires by utilizing spatial information. More details coming.

We also support a `--verbose` flag to turn on logging, if required.
...
* `mire_detection_dl.py` and `mire_detection_graph.py` : Detect mires using `dl-radial-scanning` and `graph_cluster` (for both `dl` and `image_proc`) mire localization methods, respectively 
* `camera_size.py`: Gets the actual 3D Placido locations using the placido model dimensions and camera parameters.
* `segment_mires.py` : Segments mires based on the method (`image_proc` or `dl`) used.
