...
We now support two additional flags that can be provided in the running script. These flags contain some heuristics based robust solutions that can work for images where original methodology fails:

1. `--heuristics_cleanup_flag`: Cleans up overlaps and spikes that can be seen in `plots.py`.
2. `--heuristics_bump_cleanup_flag`: Removes bumps as they appear in `plots.py`
